...
class NetworkNode(BaseModel):
    ...
    status: str = Field(description="Status: 'active', 'blocked', 'inactive'", default="active")
    ip_address: Optional[str] = Field(description="IP address if applicable", default=None)
    ...
    blocking_commands: List[str] = Field(description="Commands used to block this node", default=[])
...
class NetworkTopology(BaseModel):
    ...
    blocked_flows: List[Dict[str, Any]] = Field(description="Blocked flow entries", default=[])
    mitigation_summary: str = Field(description="Summary of applied mitigations", default="")
...
class ResponseAgent:
    ...
    def query_ryu_topology(self) -> Dict[str, List]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                topology_data["switches"] = switches if isinstance(switches, list) else []
                print(f"Found {len(topology_data['switches'])} switches: {topology_data['switches']}")
            else:
                    ...
                    if ports_response.status_code == 200:
                        ...
                        for port in ports:
                            ...
                            if port_no and port_no != 4294967294:  # Skip LOCAL port
                                if 'eth1' in port_name or (port_no == 1):  # First port typically connects to host
                                    # Infer host based on switch and port
                                    host_ip = self._infer_host_ip_from_switch_port(dpid, port_no)
                                    if host_ip:
                                        topology_data["hosts"].append({
                                            "dpid": dpid,
                                            "port": port_no,
                                            "ipv4": host_ip,
                                            "mac": "00:00:00:00:00:0" + str(len(topology_data["hosts"]) + 1)
                                        })
            ...
            if not topology_data["hosts"] and topology_data["switches"]:
                print("No hosts discovered from ports, using fallback host detection...")
                default_hosts = [
                    {"dpid": 1, "port": 1, "ipv4": "10.0.0.1", "mac": "00:00:00:00:00:01"},
                    {"dpid": 2, "port": 1, "ipv4": "10.0.0.2", "mac": "00:00:00:00:00:02"},
                    {"dpid": 3, "port": 1, "ipv4": "10.0.0.3", "mac": "00:00:00:00:00:03"}
                ]
            ...
            print(f"Discovered {len(topology_data['hosts'])} hosts: {[h['ipv4'] for h in topology_data['hosts']]}")
    ...
    def create_network_topology_graph(self, include_blocked: bool = True) -> NetworkTopology:
        ...
        for dpid in topology_data["switches"]:
            try:
                # Get switch details
                response = requests.get(f"{self.ryu_base_url}/stats/desc/{dpid}", timeout=5)
                switch_desc = response.json().get(str(dpid), {}) if response.status_code == 200 else {}
        ...
        mitigation_summary = f"Applied {len(self.applied_commands)} mitigation commands. "
        if self.blocked_hosts:
            blocked_host_names = [self._ip_to_host_name(ip) for ip in self.blocked_hosts]
